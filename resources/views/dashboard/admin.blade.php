    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 flex justify-between items-end">
            <div>
                ...
                <p class="text-slate-500 mt-2 text-lg">Welcome back, <span class="font-semibold text-slate-800">{{ auth()->user()->name }}</span></p>
            </div>
            <div class="hidden sm:block">
                <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium bg-slate-100 text-slate-900 border border-slate-100 shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <!-- Total Students -->
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow duration-300">
                ...
                <div class="mt-4 flex items-center text-sm">
                    ...
                    <span class="text-slate-500">{{ $stats['inactive_students'] }} Inactive</span>
                </div>
            </div>
            ...
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900 mt-2">৳{{ number_format($stats['total_revenue']) }}</p>
                    </div>
                ...
                <div class="mt-4 flex items-center text-sm">
                    ...
                    <span class="ml-2 text-slate-400">this month</span>
                </div>
            </div>
            ...
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-slate-500 font-medium">Today's Collection</p>
                        <p class="text-2xl font-bold text-slate-900 mt-2">৳{{ number_format($stats['today_revenue']) }}</p>
                    </div>
            ...
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-slate-500 font-medium">Outstanding Fees</p>
                        <p class="text-2xl font-bold text-slate-900 mt-2">৳{{ number_format($stats['total_outstanding']) }}</p>
                    </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            ...
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <select class="text-sm border-slate-200 rounded-xl text-slate-500 focus:ring-slate-500 focus:border-slate-500">
            ...
            <!-- Class-wise Students -->
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 p-6">
                <h3 class="text-lg font-bold text-slate-900 mb-6">Students by Class</h3>
                <div class="space-y-5 overflow-y-auto max-h-72 pr-2">
                    @foreach($classWiseStudents as $class)
                    <div class="group">
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center">
                                <span class="w-8 h-8 rounded-xl bg-slate-100 text-slate-600 flex items-center justify-center text-xs font-bold mr-3 group-hover:bg-slate-200 group-hover:text-slate-900 transition-colors">
                                    {{ substr($class->name, -2) }}
                                </span>
                                <span class="text-sm font-semibold text-slate-700">{{ $class->name }}</span>
                            </div>
                            <span class="text-sm text-slate-500 font-medium">{{ $class->students_count }} Students</span>
                        </div>
                        <div class="w-full bg-slate-100 rounded-full h-2">
                            <div class="bg-gradient-to-r from-blue-500 to-slate-600 h-2 rounded-full transition-all duration-500 ease-out" style="width: {{ ($stats['total_students'] > 0) ? ($class->students_count / $stats['total_students']) * 100 : 0 }}%"></div>
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-50 flex justify-between items-center">
                    ...
                    @can('student-list')
                    <a href="{{ route('students.index') }}" class="text-sm font-medium text-slate-800 hover:text-slate-800 transition-colors">View All</a>
                    @endcan
                </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                <div class="flex-shrink-0">
                                    ...
                                    @else
                                        <div class="h-10 w-10 rounded-full bg-slate-200 flex items-center justify-center text-slate-800 font-bold border border-slate-200">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-xs text-slate-500">{{ $student->class->name ?? 'N/A' }} {{ $student->section->name ?? '' }}</p>
                                </div>
                            </div>
            ...
            <div class="bg-slate-50 rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="p-6 border-b border-slate-50 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-slate-900">Recent Payments</h3>
                    @can('payment-list')
                    <a href="{{ route('payments.index') }}" class="text-sm font-medium text-slate-800 hover:text-slate-800 transition-colors">View All</a>
                    @endcan
                </div>
                <div class="divide-y divide-gray-50">
                    ...
                    <div class="p-6 hover:bg-slate-50 transition-colors duration-150">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-semibold text-slate-900">{{ $payment->student->full_name ?? 'N/A' }}</p>
                                <p class="text-xs text-slate-500 mt-0.5">{{ $payment->receipt_number }} • {{ ucfirst($payment->payment_method) }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-bold text-slate-900">৳{{ number_format($payment->amount) }}</p>
                                <p class="text-xs text-slate-400 mt-0.5">{{ $payment->payment_date->format('M d') }}</p>
                            </div>
                        </div>
                    </div>
                    @empty
