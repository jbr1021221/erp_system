    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
            <div class="bg-slate-50 rounded-xl shadow p-6">
                ...
                <p class="text-2xl font-bold text-slate-900 mt-2">৳{{ number_format($stats['today_collection']) }}</p>
            </div>
            
            <div class="bg-slate-50 rounded-xl shadow p-6">
                <p class="text-sm text-slate-600 font-medium">This Month</p>
                <p class="text-2xl font-bold text-slate-900 mt-2">৳{{ number_format($stats['month_collection']) }}</p>
            </div>

            <div class="bg-slate-50 rounded-xl shadow p-6">
                <p class="text-sm text-slate-600 font-medium">Month Expenses</p>
                <p class="text-2xl font-bold text-slate-900 mt-2">৳{{ number_format($stats['month_expenses']) }}</p>
            </div>

            <div class="bg-slate-50 rounded-xl shadow p-6">
                <p class="text-sm text-slate-600 font-medium">Pending Expenses</p>
                <p class="text-2xl font-bold text-slate-900 mt-2">{{ $stats['pending_expenses'] }}</p>
            </div>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-slate-50 divide-y divide-gray-200">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{{ $payment->receipt_number }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">{{ $payment->student->full_name ?? 'N/A' }}</td>
