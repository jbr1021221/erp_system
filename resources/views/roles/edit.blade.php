        ...
        <form action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white shadow-sm rounded-xl border border-gray-100 p-6">
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md md:w-1/2" {{ $role->name == 'Super Admin' ? 'readonly' : '' }}>
                    </div>
                     @error('name') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                     @if($role->name == 'Super Admin') <p class="mt-1 text-xs text-gray-500">Super Admin role name cannot be changed.</p> @endif
                </div>
            </div>
