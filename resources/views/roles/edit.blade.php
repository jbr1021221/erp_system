        ...
        <form action="{{ route('roles.update', $role->id) }}" method="POST" class="space-y-6">
            ...
            <div class="bg-white rounded-lg border border-slate-200 p-6">
                <div>
                    ...
                    <div class="mt-1">
                        <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required class="shadow-sm focus:ring-slate-500 focus:border-slate-500 block w-full sm:text-sm border-slate-300 rounded-md md:w-1/2" {{ $role->name == 'Super Admin' ? 'readonly' : '' }}>
                    </div>
                     @error('name') <p class="mt-2 text-sm text-red-600">{{ $message }}</p> @enderror
                     @if($role->name == 'Super Admin') <p class="mt-1 text-xs text-slate-500">Super Admin role name cannot be changed.</p> @endif
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-start">
                                    <div class="flex items-center h-5">
                                        <input id="permission_{{ $permission->id }}" name="permission[]" type="checkbox" value="{{ $permission->id }}" 
                                        {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }}
                                        class="permission-checkbox focus:ring-slate-500 h-5 w-5 text-slate-800 border-slate-300 rounded">
                                    </div>
                ...
                <div class="px-6 py-4 bg-slate-100 border-t border-slate-200 flex justify-end">
                     <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-500">
                        Update Role
                    </button>
                </div>
            </div>
