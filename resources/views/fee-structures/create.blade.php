...
@section('title', 'Add New Fee - ERP System')
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-lg border border-slate-200 overflow-hidden">
        <form action="{{ route('fee-structures.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="fee_type" id="fee_type" required value="{{ old('fee_type') }}" placeholder="e.g. Tuition Fee" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 bg-slate-50">
                </div>

                <!-- Amount -->
                <div class="space-y-2">
                    <label for="amount" class="block text-sm font-semibold text-slate-700">Amount (TK)</label>
                    <input type="number" step="0.01" name="amount" id="amount" required value="{{ old('amount') }}" placeholder="0.00" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 bg-slate-50">
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="academic_year" id="academic_year" required value="{{ old('academic_year', date('Y')) }}" placeholder="2024" class="block w-full rounded-xl border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-3 bg-slate-50">
                </div>

                <!-- Mandatory -->
                <div class="flex items-center space-x-3 pt-8">
                    <input type="hidden" name="is_mandatory" value="0">
                    <input type="checkbox" name="is_mandatory" id="is_mandatory" value="1" {{ old('is_mandatory', true) ? 'checked' : '' }} class="h-5 w-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
