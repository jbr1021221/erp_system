...
<div>
  ...
  <input
    ...
    {{ $required ? 'required' : '' }}
    style="width:100%;height:50px;padding:0 14px;border:1.5px solid {{ $errors->has($name) ? 'var(--accent)' : 'var(--border-color)' }};border-radius:12px;font-size:14px;background:var(--bg-base);color:var(--text-primary);outline:none;transition:border-color 0.2s,box-shadow 0.2s;font-family:'DM Sans',sans-serif;"
