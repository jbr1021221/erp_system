            ...
            <div class="p-8 pb-0">
                <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-slate-700">
                    <div class="flex items-center gap-2">
                        ...
                        <h2 class="text-xs font-semibold tracking-widest text-slate-400 uppercase">Personal Information</h2>
                    </div>
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 dark:bg-slate-700 px-3 py-1 rounded-full border border-slate-100 dark:border-slate-600 shadow-sm">
                        ID: #{{ str_pad($teacher->id, 5, '0', STR_PAD_LEFT) }}
                    </div>
                </div>
            </div>

            <div class="px-8 pb-8 pt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Name -->
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" value="{{ old('name', $teacher->name) }}" required placeholder="e.g. John Doe" 
                           class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-transparent dark:border-slate-600 rounded-2xl text-sm font-bold text-slate-700 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-emerald-500 transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <select name="is_active" required class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-transparent dark:border-slate-600 rounded-2xl text-sm font-bold text-slate-700 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-emerald-500 transition-all">
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <input type="file" name="photo" accept="image/*" 
                               class="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-transparent dark:border-slate-600 rounded-2xl text-sm font-bold text-slate-700 dark:text-slate-100 transition-all file:mr-4 file:py-1.5 file:px-3 file:rounded-lg file:border file:border-slate-300 dark:file:border-slate-600 file:text-sm file:text-slate-600 dark:file:text-slate-300 file:bg-transparent file:cursor-pointer hover:file:bg-slate-50 dark:hover:file:bg-slate-700">
                    </div>
                ...
                <div class="space-y-2 md:col-span-2">
                    ...
                    <textarea name="address" rows="3" placeholder="Residential address..." 
                              class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-transparent dark:border-slate-600 rounded-2xl text-sm font-bold text-slate-700 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 focus:ring-2 focus:ring-emerald-500 transition-all">{{ old('address', $teacher->address) }}</textarea>
                    @error('address') <p class="text-xs text-red-500 font-bold mt-1 ml-1">{{ $message }}</p> @enderror
                </div>
            </div>

            <div class="px-8 py-6 bg-slate-50/50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-700 flex items-center justify-end gap-3">
                <a href="{{ route('teachers.index') }}" class="bg-transparent border border-slate-600 text-slate-300 hover:bg-slate-700 rounded-lg px-4 py-2 text-sm font-medium transition-all">
                    Cancel
                </a>
                <button type="submit" class="bg-emerald-600 text-white border border-transparent hover:bg-emerald-700 rounded-lg px-4 py-2 text-sm font-medium transition-all">
