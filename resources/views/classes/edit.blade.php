...
@section('content')
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between mb-6 pb-5 border-b border-slate-200 dark:border-slate-700">
            <div>
                <h1 class="text-xl font-semibold text-slate-800 dark:text-slate-100">Edit Class: {{ $class->name }}</h1>
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Update class details below.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div class="px-6 pt-6 pb-0">
                <div class="flex items-center justify-between pb-3 mb-4 border-b border-slate-200 dark:border-slate-700">
                    ...
                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 dark:bg-slate-700 px-3 py-1 rounded-full border border-slate-100 dark:border-slate-600">
                        ID: #{{ str_pad($class->id, 5, '0', STR_PAD_LEFT) }}
                    </span>
                </div>
            </div>

            <form action="{{ route('classes.update', $class) }}" method="POST" class="px-6 pb-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-y-5 gap-x-4 sm:grid-cols-6">
                    ...
                    <div class="sm:col-span-6">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="block w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-colors resize-vertical">{{ old('description', $class->description) }}</textarea>
                    </div>
                    ...
                    <div class="sm:col-span-6">
                        <div class="flex items-start gap-3">
                            <div class="flex items-center h-5 mt-0.5">
                                <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', $class->is_active) ? 'checked' : '' }}
